#!/usr/bin/env node
/**
 * Bootstrap a fresh Discovery project: 'Molecule3D-School'.
 *
 * Subject: школьники изучают молекулы — веб-приложение, отрисовывающее 3D-модель
 * молекулы (например, H2O, CH4, этанол). Цель Discovery — снять неопределённость
 * по MVP-границам, аудитории, технологическому стеку 3D-визуализации в браузере.
 *
 * Usage: node bootstrap-molecule3d.mjs
 */
...
const PROJECT_NAME = 'Molecule3D-School';
...
const EPIC_NAME = 'Molecule3D Discovery';
const EPIC_DESC = PROJECT_DESC;

const db = new Database(DB_PATH);
db.pragma('foreign_keys = ON');

// Check no active project with this name
...
const tx = db.transaction(() => {
  ...
  // 5. Empty episode_workflows row (engine expects this).
  // NOTE: track is NOT NULL on write — must be 'formal', not NULL.
  // (This was a known footgun in older bootstrap scripts.)
  db.prepare(
