...
**Статус:** **fixed** (2026-08-16 ~01:30). Изоляция доведена: ledger-трейсы живы; воркер САМ сшил snapshot-подобный payload (трейсы без traceId), материализатор пропустил его по форме. Фикс: строгая каноничность в pass-through (числовой traceId/traceHash у каждого trace), иначе пересборка из ledger. Тест+smoke зелёные, коммит ушёл.

### GB-9 — вторая выборка (P06, 04:10): ДРУГОЙ класс — штатная пауза бюджета планировщика

P06 (units) Development 15/17: гейт `development-plan-task-graph` отверг продукт
планировщика 3× подряд (`repair_required`, final; диагност `invalid-input-contract`,
«DevelopmentCase failed contract validation») → repair-бюджет исчерпан → explicit-pause.
**Это ЛЕГИТИМНЫЙ fail-closed по дизайну** (карточка planning.decomposition, воркер не
смог построить контрактно-валидный DevelopmentCase за 3 попытки) — материал по качеству
плана, НЕ движковый дефект. Открытой аномалией GB-9 остаётся только выборка P02
(карточка #109: гейт ACCEPTED → пауза БЕЗ эффекта) — она ждёт третьей выборки.

### Материал: P07 (snake) — acceptance-ячейка исчерпала 5 попыток (04:43)

Snake (иерархические AC-коды AC-NFR-x.y) — acceptance-гейт 5× repair_required → пауза.
Штатный бюджетный цикл (как у P06-планировщика). Причина отказов — см. диагносты
гейта (выше в логе сессии). Recovery: factory.mjs continue/утренний проход.
Не движковой дефект — материал по стресс-устойчивости acceptance-гейта на вложенных кодах.
