...
| **2** | Immutable Installation & Registries | 0.5 (Phases 2,3 excl. live LM driver) | 8 | ✅ DONE (8/8 lanes, 140 tests; R-07 resolved) | integrator + 1 SQL owner | `2dd386c` → checkpoint | installed bytes replay after source mutation; released version digest immutable; pinned installation not nullifiable; 3rd module installs without catalog edit |
| **3** | Durable Execution Primitives | 0.6 (Phase 4) | 8 | ✅ DONE (8/8 lanes, 464 tests) | integrator | `f0367d1` → checkpoint `a46d3ea` | crash after worker completion resumes from exact receipt+product; no latest-execution/process-scope/task-metadata/magic-binding reconstruction |
