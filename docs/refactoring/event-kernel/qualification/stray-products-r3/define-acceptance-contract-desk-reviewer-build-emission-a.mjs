 ...
 * define-acceptance-contract-desk-reviewer-verify-out-emission-a.json (99 independent
 * recomputations through the REAL kernel cell surfaces) - nothing is
 * asserted that the verify run did not establish.
 *
 * EMISSION A collision-free copy: rebuilds the first reviewer emission
 * under -emission-a filenames after the canonical namespace was
 * overwritten in place by a concurrent writer mid-review. Deterministic;
 * consumes only define-acceptance-contract-desk-reviewer-verify-out-emission-a.json.
 *
 * Run: node define-acceptance-contract-desk-reviewer-build-emission-a.mjs
 */
...
const verifyRun = JSON.parse(readFileSync(join(DIR, 'define-acceptance-contract-desk-reviewer-verify-out-emission-a.json'), 'utf8'));
...
writeFileSync(join(DIR, 'define-acceptance-contract-desk-reviewer-verification-emission-a.json'), JSON.stringify(vv, null, 2) + '\n');
...
writeFileSync(join(DIR, 'define-acceptance-contract-desk-reviewer-review-emission-a.json'), JSON.stringify(fr, null, 2) + '\n');
...
writeFileSync(join(DIR, 'define-acceptance-contract-desk-reviewer-trace-emission-a.json'), JSON.stringify(rt, null, 2) + '\n');
...
writeFileSync(join(DIR, 'define-acceptance-contract-desk-reviewer-product-submission-emission-a.json'), JSON.stringify(fsArt, null, 2) + '\n');
...
`;
writeFileSync(join(DIR, 'define-acceptance-contract-desk-reviewer-submission-summary-emission-a.md'), md);
