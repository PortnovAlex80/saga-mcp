/**
 * derive-system-requirements desk (author) - deterministic builder (r3).
 *
 * r3 round: a fresh execution of the SAME desk task (identical task
 * projection, identical accepted upstream material carried over verbatim
 * from the r2 round). Deterministic authoring law applies: pinned
 * timestamps, no clock reads, no randomness - the authored artifacts MUST
 * reproduce the r2 content digests byte-exactly. Any digest drift is an
 * r3 failure, not a new revision.
 *
...
console.log(JSON.stringify({
  built: 'derive-system-requirements desk (author) r3 artifacts',
