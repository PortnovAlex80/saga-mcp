...
| `modules/**` resources (61 content files, enumerated §B.12) | Installed skills/checklists/trackers/call-templates | RETAIN-AND-MOVE (content) | semantic content re-hosted in installed manifests by WP-11D/F/V/L; **old src paths deleted at EK-8** |
| `infrastructure/workplace-settlement-drain.ts`, `shared/**` (4) | Settlement drain + shared snapshots | EK-8 | aggregate settlement (EK-6) |
...
| `lifecycle-pipeline/**` (4), `docs-graph/**` (10) | Pipeline view + docs graph tool | EK-8 | projection views over the new read API (WP-10); docs-graph is tooling, re-pointed or deleted by WP-10 owner decision |

## D. `scripts/` and `tools/` operational surface

| Path(s) | Role | Disposition | Phase | Replacement |
|---|---|---|---|---|
| `scripts/factory.mjs`, `factory-engine-spawn.mjs`, `factory-bootstrap*.mjs`, `factory-status.mjs`, `hex-composition.mjs`, `bootstrap-*` (7 stage/testbed bootstraps), `check-*.mjs`, `real-factory-smoke.mjs`, `restore-from-checkpoint.mjs`, `testbed-*.mjs` (6), `quality-summary.mjs`, `wasted-turns-metric.mjs`, `lib/**` | Old factory operators/testbed drivers over the old schema and checkpoint restore | DELETE | EK-8 | new run drivers under the runbook (EK-11 kit commands); old testbed bootstraps have no successor |
...
| `tests/architecture/**` (80) | Dependency-direction allowlists, digest-consistency, admission-distance, authority-recency, legacy-expansion-freeze ratchets | KEEP (invariant ratchets; re-pinned to the post-cutover tree by EK-8/EK-9 owners — ADR-095 precedent §"Six existing blocker suites to UPDATE (never weaken)") | — |
...
| `tests/infrastructure/**` (70) | Old repository/adapter tests | SPLIT (invariant → new repos; mirroring → DELETE) | EK-9 |
...
## G. ADR-053 material contracts — RETAIN-AND-MOVE (verified by import analysis)

The plan permits RETAIN-AND-MOVE **only** for pure contracts with one owner and no import from a deleted runtime. Verified 2026-08-24 on the base tree (grep of the full import block of each file):

| File | Chain member | Import analysis result | Verdict |
|---|---|---|---|
| `src/shared/canonical-json.ts` | canonical serialization/digest used by every contract below | imports `node:crypto` + **type-only** `DiscoveryProposalPayload` (erased at runtime) | RETAIN-AND-MOVE (moves first; everything below depends on it) |
| `src/process-modules/domain/workplace/workplace-ref.ts` | Workplace identity (ADR-053 material coordinates) | **zero imports** | RETAIN-AND-MOVE |
| `src/process-modules/domain/workplace/workplace-production-revision.ts` | `WorkplaceProductionRevision` — the accepted-material authority itself | imports only `shared/canonical-json.js` | RETAIN-AND-MOVE |
| `src/process-modules/domain/workplace/candidate-set.ts` | `CandidateSet` (author/reviewer presentation bound to one revision) | imports `canonical-json`, `workplace-ref`, **type-only** `ProductRef` from `spi/index.js` | RETAIN-AND-MOVE |
| `src/process-modules/domain/workplace/gate.ts` | `GateDecision` (decision over exact CandidateSet + CheckPlan) | imports `canonical-json`, `workplace-ref`, **type-only** `CandidateSet`/`ProductRef` | RETAIN-AND-MOVE |
| `src/process-modules/domain/workplace/accepted-authority-head.ts` | accepted-authority head contract | **zero imports** | RETAIN-AND-MOVE |
| `src/process-modules/domain/workplace/check-diagnostic.ts` | gate finding/diagnostic contract | imports only `canonical-json` | RETAIN-AND-MOVE |
...
- `EffectReceipt`: the only typed surfaces live in `src/process-modules/application/post-acceptance-effects.ts` (defines `AcceptedCandidateAuthority` — imports `observability/run-journal.js`, a deleted runtime) and in persistence/runtime files. **Fails the purity test → not RETAIN-AND-MOVE as a file**; the `AcceptedCandidateAuthority`/effect-receipt *contract* is extracted into the new kernel domain package as part of the move of §G files (WP-05), and effect application is re-implemented as obligation-driven effects (WP-07/WP-08).
- `CellFinalAcceptance`: exists only as SQLite persistence (`src/infrastructure/workplace/sqlite-cell-final-acceptance.ts`, imports `post-acceptance-effects` + `domain/recovery` + better-sqlite3). **Fails the purity test → DELETE @ EK-8**; new relation + repository per EK-1/EK-3.
- `production-cell-reducer.ts` / `workplace-state.ts` are pure (import only each other) but encode the **old loop protocol** (loop states + Kanban mirroring), not ADR-053 material contracts → DELETE @ EK-8, replaced by the new `Workplace` reducer (WP-05).

One-owner verification: each §G file has exactly one defining module (verified by `grep -rln` over `src/`); their many importers (`factory-start.ts`, `products.ts`, repositories, check providers) are all classified DELETE above — importer death does not affect contract purity.
...
4. CC-U2 warrant-oracle command authority (reserved ADR-093) — `tools/cc-proof-hosting-registry.mjs` + `src/infrastructure/verification/warrant-oracle-adapters.ts` stay until ADR-093 resolves; classify at EK-13.
...
- `src/`: 572 tracked files = **7 RETAIN-AND-MOVE code contracts** (§G) + **61 RETAIN-AND-MOVE content files** (§B.12: 55 `package/resources/**` + 5 formalization `use-case/resources/**` + `WORKSHOP.md`) + **504 DELETE** (of which 5 are EK-7-class scheduling-surface files: `work-assignment-core.ts`, `dispatcher.ts`, `tasks.ts`, `conveyor-runtime-helper.ts`, `conveyor-runtime.ts`; the rest EK-8). 0 unclassified, 0 both-DELETE-and-RETAIN.
