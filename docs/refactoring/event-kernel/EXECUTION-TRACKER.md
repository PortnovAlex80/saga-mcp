...
| WP-02 | Forward graph (inputs/commands only; PRIVATE until both frozen) | EK-0 | subagent | **DONE — frozen (reconciled at EK-1 exit)** | `1ccbf66d` | coordinator (reconciled at EK-1 exit) | forward-graph.json sha256 a3800721…, 56 nodes / 132 edges / 10 gaps G1-G10 | names frozen at EK-1 exit |
| WP-03 | Reverse graph (terminal claims only; PRIVATE) | EK-0 | subagent | **DONE — frozen (reconciled at EK-1 exit)** | `6e029e08` | coordinator (reconciled at EK-1 exit) | reverse-graph.json, 88 nodes / 112 edges / 28 proofs (23 closed, 5 gap-flagged) / 8 gaps G1-G8 | names frozen at EK-1 exit |
| WP-04 | Legacy + document deletion manifests | EK-0 | subagent | **DONE** | `681a82e7` | coordinator review CLOSED at EK-1 exit (receipt d491ce7c) | legacy: 127 tables + 572 src files (504 DELETE; purity-tested ADR-053 set) / docs: 439 entries (177/14/248), zero unclassified | family-level test split deferred to EK-8/9 |
...
| WP-16 | Freeze + validate the three admission specs | WP-01..04 | 3 spec agents | **DONE** — **16b DONE** (394be77d: schema+manifest b1ef94c2, 4/4 mutations killed); **16c DONE** (1258cec7, 21/21 det., 3 RED); 16a rev3 DONE (b507773a, `ek1/fix-complexity-residual-dims`: 8 census-frozen dims re-bound, budget rev3, vector v3, 8/8 synthetic reds) — coordinator freeze DONE at EK-1 exit | ek-admission validator (blocking, ALL GREEN) | validate:ek-admission-specs + EK-ADMISSION-RECEIPT.json | package.json wiring = coordinator EK-1 exit; §7 items 9-10 resolved at EK-8 (single accountant, ACD converged 8665ed0f…) |
...
Suite commands COMPLETE — counts/durations recorded in BASELINE.md (EK-0 closed).
