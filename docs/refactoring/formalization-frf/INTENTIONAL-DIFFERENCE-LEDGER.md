...
| D-14 | Stale reachability note | `src/workflow-kernel/workshops/formalization/index.ts` header still says the package is "reachable ONLY from focused tests … until WP-12 performs the atomic production cutover". The cutover LANDED (production.ts imports `workshops/formalization/roles.js`); the note is stale documentation. `structure.test.mjs` still asserts the pre-cutover reachability law. FRF-11 docs pass + test rewrite. |
