...
| LIVE-SCOPE-003 | Run 022 correctly rejected the first root implementation CandidateSet for violating its frozen write scope, but the replacement author received only `development.implementation-scope.v1:failed` and could not see the offending Git paths or the authoritative scope | the deterministic scope provider returned a bare outcome even though the universal Gate-to-Recovery path already preserves typed diagnostics | every scope/binding failure now emits a content-addressed diagnostic; regressions assert exact Git-vs-claim drift and out-of-authority paths | fixed in code; host reload required |
...
| LIVE-HANDOFF-001 | Run 020 accepted the ballistic-calculator Discovery Proposal, then Formalization invented and approved an unrelated communication bridge | lifecycle handoff preserved only the Discovery certificate/outcome and discarded the accepted semantic product plus original initiative subject; scripted lifecycle fixtures encoded Discovery `output: null` | Discovery Proposal becomes exact ProcessRun output; lifecycle maps ref/hash/payload and initiative subject; Formalization validates all four; canonical E2E asserts byte-for-byte information conservation | fixed in code; Run 020 preserved as incident evidence and clean Run 021 required |
| TEST-FIXTURE-002 | Three package/replay temporal cases paused at local runnability while the other 28 temporal cases passed | those cases provisioned a README-only Git repository even though Product Build now requires exact `test` and `start` commands plus loopback health; the fixture still modeled the pre-ADR-058 lifecycle | package/replay fixture now contains the same minimal runnable Node contract as the canonical fresh-repo fixture; local-runnability remains real and is not mocked | fixed; focused temporal package/replay file and full suite rerun required |
