...
| CC-GAP-7 (deliverable-aware oracle) | Execution slice INTEGRATED from `d762ce52` + provenance pin `18b7e444`: provider `1.15.0`, manifest contract `1.2.0`, DB-only warrant cross-binding, typed oracle-insufficient separation, adapter execution and warrant/adapter/environment receipt binding with blocking mutations. NOT EXIT: adapter declarations remain candidate-produced rather than pinned to an InstalledWorkshopManifest/package digest; CC-U2 non-circular command authority is open. |
