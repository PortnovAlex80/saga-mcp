...
| C5 | git integration `ORDER BY t.id DESC` and `ORDER BY gd.decided_at DESC` | **CONFIRMED → review-decision FIXED; task-binding BLOCKED (design conflict)** | review-decision recency removed (exact `workplace+subject+gate_phase='final'+verdict='accepted'`, dropped `ORDER BY gd.decided_at DESC`). Task-binding: the QA's suggested fix (`JOIN tasks t ON t.id = s.task_id`, the submission's origin task) was implemented by the operator in `694d80d` but **breaks carry-forward** — `production-cell-integration-candidate-binding.test.mjs` now fails `PRODUCTION_CELL_INTEGRATION_TASK_MISSING`, because a carried-forward candidate's submission.task_id is the ORIGIN process's task, not the current workplace's task the integration must update. Correct fix: track the accepted-author **task_id** on the `factory_accepted_authority_head` (C1) and select the task from it — not `s.task_id` (wrong for carry-forward) and not `ORDER BY t.id DESC` (recency). Left open to avoid conflicting with the operator's active git-integration work. |
...
| C10 | CheckProvider implementation digest not actually verified | **CONFIRMED → FIXED in tranche 3** | `providerDigest` added to the `CheckProvider` interface; every concrete provider (product-contract, srs-structural, review-verdict, discovery×2, development×3, accessible-counter×2, local-runnability) now carries its pinned digest; the driver verifies `provider.providerDigest === entry.check.providerDigest` (version match alone was insufficient) |
...
| C12 | GateRun replay re-runs providers, regresses terminal→checking | **CONFIRMED → FIXED in tranche 3** | `driveGateRun` early-returns the persisted terminal decision + receipts via `readTerminalDecisionForGateRun` (added to the GateRunDriverRepo interface + both impls) without re-running providers or regressing state; a GateRun is now one-shot |
...
| C14 | revision assembled from `parent:null` + single execution's products (not cumulative) | **CONFIRMED → FIXED in tranche 4** | `assembleRevisionFromProducts` now applies the contribution as a delta on the workplace's current accepted-author revision (read from the C1 authority head) instead of `parent:null`; the revision is a cumulative desk state. Domain regression proves X+Y (one execution) ≡ X-then-Y (two executions) converge to the same semanticDigest + member set. `put` operations keep single-execution-complete seals unaffected (full set overrides the parent). |
