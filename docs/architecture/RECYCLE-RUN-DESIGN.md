...
> Уточнение формы хука (оператор): хук — это ДВЕРЬ ЗАВОДА, не голова
> рабочего. «Дернуть завод» = операторская команда запуска (CLI/API, род
> change вместо idea): завод валидирует → FactoryOrder(change) + новый ран;
> воркеры про «хук» не знают ничего — они видят обычный вход своего цеха
> (кейс с findings-реестром, IDs, диспозициями, кодовой базой). Осталось за
> кадром E9-реализации: конкретная CLI-поверхность команды (одна строка).
