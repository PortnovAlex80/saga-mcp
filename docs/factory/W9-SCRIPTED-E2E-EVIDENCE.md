...
3. **Pre-existing-red tests (CI-02 quarantine at evidence time):** `development-task-graph-diagnostics` (stale producerExecutionRef mock), `submission-validator-diagnostics` (assertion drift), `worktree-isolation` (C5-cutover seed). The worker-done completion boundary test was reconciled during ADR-073 and is no longer quarantined.
