...
test('BLOCKING MUTATION (f): the trusted_providers migration requires the EXACT version→built-in-digest pair — authentic 1.12.0/1.13.0/1.14.0 (and the older lineage) migrate, a FORGED basis on a known version is drift, never laundered', () => {
  ...
  for (const baseline of ['1.1.0', '1.12.0', '1.13.0', '1.14.0']) {
    assert.throws(
  ...
  // The CROSS pair — 1.14.0's version with 1.13.0's authentic digest — is
  // just as foreign: the pair, not the version alone, is the authority.
  assert.throws(
    () => migrateFromBaseline('1.14.0', `built-in:${AUTHENTIC_BASELINES['1.13.0']}`),
    /LOCAL_RUNNABILITY_TRUST_POLICY_DRIFT/u,
...
test('BLOCKING MUTATION (f): an unmigrated/foreign trust row is rejected (fail closed)', () => {
  ...
  try {
    assert.throws(
      () => ensureLocalRunnabilityProviderTrust(db),
      /LOCAL_RUNNABILITY_TRUST_POLICY_DRIFT/u,
      'an unmigrated trust row fails closed — a ship without the migration is fenced out',
    );
  } finally {
    db.close();
  }
