...
test('бюджет ядра: интерпретатор остаётся маленьким', () => {
  ...
  // 3700 (приёмка веера по рабочим местам). Последний шаг вернул конвейеру
  // его же правило: стол — это место работы, и на него нанимают следующего
  // рабочего, пока работа не принята.
  ...
  assert.ok(lines < 3700, `ядро выросло до ${lines} строк — повод для ADR, а не для исключения`);
