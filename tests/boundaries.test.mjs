...
test('бюджет ядра: интерпретатор остаётся маленьким', () => {
  ...
  // 2026-08-31: 3400 → 3500 (семантика таймаутов) → 3600 (touchProgress) →
  // 3700 (приёмка веера по рабочим местам) → 3900 (эскалация на уровень выше).
  // Предпоследний шаг вернул конвейеру его же правило: стол — это место
  // работы, и на него нанимают следующего рабочего, пока работа не принята.
  // Последний убрал главного потребителя ручного труда: исчерпав ремонт,
  // гейт идёт не к человеку, а к столу-поставщику, а устаревший результат
  // ниже по конвейеру переоткрывается сам. Человек остался там, где он
  // незаменим: идея на входе, недостающий внешний материал, бюджет.
  ...
  assert.ok(lines < 3900, `ядро выросло до ${lines} строк — повод для ADR, а не для исключения`);
