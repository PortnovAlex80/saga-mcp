...
test('бюджет ядра: интерпретатор остаётся маленьким', () => {
  ...
  // 2026-08-31: 3400 → 3500 (объяснение семантики таймаутов) → 3600
  // (touchProgress: обновление операционного окна без события). Второй шаг —
  // настоящая механика, и он осознан: живая строка стоила бы сотни событий в
  // журнале, если бы шла через сердцебиение.
  ...
  assert.ok(lines < 3600, `ядро выросло до ${lines} строк — повод для ADR, а не для исключения`);
