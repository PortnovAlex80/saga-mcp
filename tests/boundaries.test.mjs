...
test('бюджет ядра: интерпретатор остаётся маленьким', () => {
  ...
  // 2026-08-31: поднято с 3400 — рост пришёлся на объяснение семантики
  // таймаутов (почему сторож один и почему бюджет не бюджет), а не на новую
  // механику. Комментарий, объясняющий решение, дешевле самого решения.
  ...
  assert.ok(lines < 3500, `ядро выросло до ${lines} строк — повод для ADR, а не для исключения`);
