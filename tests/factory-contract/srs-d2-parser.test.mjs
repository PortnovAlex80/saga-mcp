...
// These guard against the three bugs that caused GLM-4.7 to loop on the
// architecture gate: heading-level truncation, fenced-code-block requirement,
// and YAML-list-only stanza start.
...
test('D2 subsection format: ### AC-1 with bare ac: fields parsed correctly', () => {
  const srs = `
## §D2: AC Decomposition
...
test('D2 with incidental fenced snippet does not hide raw stanzas', () => {
  // The model may include a pattern legend or code example inside D2.
  // The parser must still find the real stanzas in the raw section text.
  const srs = `
## §D2: AC Decomposition
...
`;
  const stanzas = extractD2Stanzas(srs);
  assert.equal(stanzas.length, 1, 'incidental fenced snippet must not hide raw stanzas');
  assert.equal(stanzas[0].ac, 'AC-1');
});
...
test('D2 section boundary: ### subsections inside ## D2 do not truncate section', () => {
  // This is the exact bug: ## §D2 heading, followed by ### AC-1 subsections.
  // The old parser matched ### as "next heading" and truncated to 0 stanzas.
  const srs = `
## §D2 AC Decomposition
...
`;
  const stanzas = extractD2Stanzas(srs);
  assert.equal(stanzas.length, 2, 'subsection headings inside D2 must not truncate the section');
});
