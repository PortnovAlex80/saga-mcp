...
async function setupFreshDb(repoPath, baseCommit) {
  ...
  // fields are rejected fail-closed before the golden path can start.
...
test('Factory transition conformance: reject -> repair -> accept, then replay with zero scripted calls', { timeout: 540000 }, async () => {
  ...
  try {
    ...
    assert.ok(runAInvocations.some(i => i.key?.module === 'solution-development@1.4.4'), 'Run A Development used scripted physical workers');
