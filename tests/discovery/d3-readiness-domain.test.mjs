...
test('D3 domain: valid complete assessment passes', () => {
  const r = validateReadinessAssessment(validAssessment(), PROPOSAL_HASH, ALLOWED_REFS);
...
test('D3 domain: missing dimension is rejected', () => {
  ...
  const r = validateReadinessAssessment(a, PROPOSAL_HASH, ALLOWED_REFS);
...
test('D3 domain: proposal_id in payload is rejected (physical ids are not semantic content)', () => {
  // v2 contract: the payload binds by content hash only; a physical proposal
  // row id must be rejected so capsule payloads stay lifecycle-stable.
  const a = validAssessment();
  a.proposal_id = 42;
  const r = validateReadinessAssessment(a, PROPOSAL_HASH, ALLOWED_REFS);
  assert.equal(r.valid, false);
  assert.ok(r.errors.some(e => e.includes("'proposal_id' must not appear")));
});

test('D3 domain: proposal_content_hash mismatch is rejected (immutable target binding)', () => {
  const r = validateReadinessAssessment(
    validAssessment(), 'b'.repeat(64), ALLOWED_REFS,
  );
  assert.equal(r.valid, false);
