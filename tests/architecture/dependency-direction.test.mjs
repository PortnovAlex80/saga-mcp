...
//     2026-08-02  baseline = 8 → 0  (Wave 7 re-check: physical extraction of the
//                8 SQLite adapters to src/infrastructure/process-modules/
//                temporarily introduced 8 pure re-export shims at the old
//                module paths to keep import resolution during the
//                parallel-agent refactor window. Third audit required the
//                shims deleted and every importer (4 src/ consumers + ~11
//                tests) migrated to the infrastructure path. All shims removed;
...
const KNOWN_VIOLATIONS = [
  ...
  // infrastructure any longer, so the 8 Rule 2 edges and their allowlist
  // entries are gone (ALLOWLIST_BASELINE returns to 0). The physical-placement
  // gate (tests/architecture/no-sqlite-in-modules.test.mjs) independently
  // forbids any sqlite substrate — type-only or runtime — under modules/.
