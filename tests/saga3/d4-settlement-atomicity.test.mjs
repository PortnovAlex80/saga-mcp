...
function fixture() {
  ...
  // Minimal FK chain so the certificate row (which REFERENCES saga3_proposals +
...
test('atomic issuance: transaction failure after certificate insert rolls back (no cert, settlement not issued)', async () => {
  ...
  try {
    ...
    const s = db.prepare('SELECT status FROM saga3_discovery_settlements WHERE id=?').get(settlement.id);
...
test('atomic reconcile: settlement.rationale changed -> reconcile rejected, status stays computed', async () => {
  ...
  try {
    ...
    db.prepare("UPDATE saga3_discovery_settlements SET status='computed' WHERE id=?").run(settlement.id);
    // TOCTOU: change rationale before the reconcile tx.
    db.prepare('UPDATE saga3_discovery_settlements SET rationale=? WHERE id=?').run('TAMPERED', settlement.id);
    let threw = false;
    try {
