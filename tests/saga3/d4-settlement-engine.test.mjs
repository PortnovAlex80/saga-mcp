...
// ---- P1: engine recovery resume / no-respawn (correction 5) ----

/**
 * Build a recovery engine: prepareIntentForExecution returns state='done' (so
 * the engine takes the recovery path), and the runtime exposes the readiness
 * control/assessment read methods the recovery path needs.
 */
async function runRecoveryEngine({ controlStatus, assessment }) {
  const proposal = { id: 50, payload: validPayload('go'), content_hash: 'h'.repeat(64), provenance: null };
  const intent = { id: 1, epic_id: 10, kind: 'discovery', objective: 'o', authority_scope: {}, output_schema: 'saga3.work-intent.discovery.v1', projected_task_id: 100, status: 'concluded', created_at: 't' };
  const runtime = {
    readEpicObjective: () => ({ name: 'e', description: 'discover' }),
    readOpenIntent: () => null, // engine will createIntent below
    createIntent: () => intent,
    setProjectedTask: () => {},
    ...
    ensureNormalizationControl: () => ({ controlIntentId: 1, sourceSubmissionId: 1, controlStatus: 'concluded', authorityIntentId: 2, authorityIntentStatus: 'concluded', taskId: 100 }),
    setControlIntentStatus: () => true,
    ensureReadinessControl: () => ({ controlIntentId: 1, proposalId: 50, proposalContentHash: 'h'.repeat(64), controlStatus: 'open', authorityIntentId: 2, authorityIntentStatus: 'open', taskId: 101 }),
    setReadinessControlStatus: () => true,
    readLatestReadinessAssessment: () => assessment,
    // Recovery-path readiness control read.
    readReadinessControlForProposal: () => ({ id: 1, epic_id: 10, kind: 'AssessDiscoveryReadiness', proposal_id: 50, proposal_content_hash: 'h'.repeat(64), source_intent_id: 1, authority_intent_id: 2, projected_task_id: 101, status: controlStatus, created_at: 't', updated_at: 't' }),
    readWorkIntent: (id) => ({ id, epic_id: 10, kind: 'discovery.assess', objective: 'o', authority_scope: {}, output_schema: 'saga3.readiness-assessment.v1', token_budget: 0, retry_budget: 0, projected_task_id: 101, status: 'concluded', created_at: 't', updated_at: 't' }),
    ...
    markSettlementFailed: () => {},
    readCertificateForSettlement: () => null,
    issueCertificateAtomically: () => ({ record: { id: 2, certificate_hash: 'f'.repeat(64) }, inserted: true }),
    reconcileExistingCertificate: () => ({ id: 2, certificate_hash: 'f'.repeat(64) }),
  };
  const executor = makeFakeExecutor(() => {});
  const readinessCalls = [];
  const readinessService = {
    async assess(req) {
      readinessCalls.push(req);
      return {
        success: true, cycles: 3, error: null,
        shadow: {
          status: 'completed', authority: 'shadow_advisor',
          assessmentId: 99, assessmentHash: 'd'.repeat(64),
          overallReadiness: 'ready', recommendedNextAction: 'proceed_to_settlement', error: null,
  ...
  const settlementService = {
    async settle(req) {
      ...
      return {
        status: 'issued', settlementId: 1, certificateId: 2, certificateHash: 'f'.repeat(64),
        policyVersion: 'saga3.discovery-settlement-policy.v1', policyHash: 'p'.repeat(64),
  ...
  const result = await engine.run({ projectId: 1, epicId: 10, concurrency: 1 });
  return { result, readinessCalls, settleCalls };
}

test('D4 engine recovery: paused readiness control without accepted assessment resumes advisor (assess called once)', async () => {
  // Control is paused, no accepted assessment -> the recovery path must RESUME
  // the advisor via readinessService.assess() (not immediately settle with a
  // paused shadow).
  const { readinessCalls, settleCalls } = await runRecoveryEngine({
    controlStatus: 'paused',
    assessment: null, // no assessment row
  });
  assert.equal(readinessCalls.length, 1, 'readinessService.assess must be called once to resume the paused advisor');
  assert.equal(settleCalls.length, 1, 'settlement must run after resume');
  // The settle() request receives the resumed shadow (completed), not a paused one.
  assert.equal(settleCalls[0].readiness.status, 'completed');
});

test('D4 engine recovery: accepted assessment already exists -> advisor NOT respawned (assess not called)', async () => {
  // An accepted assessment exists -> reconstruct the completed shadow from the
  // durable row; the advisor must NOT be re-run.
  const acceptedAssessment = {
    id: 7, control_intent_id: 1, proposal_id: 50, proposal_content_hash: 'h'.repeat(64),
    task_id: 101, execution_id: 'exec', payload: {}, content_hash: 'd'.repeat(64),
  ...
  assert.equal(readinessCalls.length, 0, 'readinessService.assess must NOT be called when an accepted assessment exists');
  assert.equal(settleCalls.length, 1, 'settlement must run with the reconstructed shadow');
  // The settle() request receives the reconstructed completed shadow.
  assert.equal(settleCalls[0].readiness.status, 'completed');
  assert.equal(settleCalls[0].readiness.assessmentId, 7);
});
