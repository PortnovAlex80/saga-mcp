...
function makeRunner({
  ...
  return new ClaudeBoardRunner({
    ...
    claimTask: ({ worker_id }) => {
      if (claimedFlag.value) return { task: null, skill: null };
      const task = states.get(STATE_TASK_ID);
      if (!task) return { task: null, skill: null };
      claimedFlag.value = true;
      states.set(STATE_TASK_ID, { ...task, assigned_to: worker_id });
      return {
        task: { ...task, tags: task.tags ?? '[]' },
        skill: task.__skill ?? 'saga-developer',
        repository: null,
        // NOTE: execution_id is intentionally OMITTED. When present, launch()
        // calls markExecutionRunning which needs a real OS process birth token
        // for the child pid — unavailable for fake EventEmitter children. The
        // W5-A6 fixes under test (prompt assembly + argv) do not depend on the
        // execution fence. execution_context.authority.allowed_saga_tools IS
        // supplied (without execution_id) so the §13.17 --allowedTools path is
        // exercised.
        execution_context: {
...
test('W5-A6 fix #1 (§0.2.7/§10.12): launch spec resolveSkill resolves the pinned installation path, NOT the global skill root', async () => {
  ...
  try {
    runner.start({ projectId: 70, concurrency: 1 });
...
test('W5-A6 fix #2 (§13.18): review task with a launch-spec reviewSkill inlines the REVIEWER skill, not the author semanticSkill', async () => {
  ...
  try {
    runner.start({ projectId: 71, concurrency: 1 });
...
test('W5-A6 fix #2 (§13.18): non-review task still uses the author semantic skill even when a reviewSkill is declared', async () => {
  ...
  try {
    runner.start({ projectId: 72, concurrency: 1 });
...
test('W5-A6 fix #3 (§13.17): launch spec allowedToolIds narrows the granted Claude builtins', async () => {
  ...
  try {
    runner.start({ projectId: 73, concurrency: 1 });
...
test('W5-A6 fix #3 (§13.17): no launch spec allowedToolIds → legacy default builtin set preserved', async () => {
  ...
  try {
    runner.start({ projectId: 74, concurrency: 1 });
...
test('W5-A6 legacy preservation: no resolveLaunchSpec callback → full legacy path (global skill root, author skill for review, default builtins)', async () => {
  ...
  const runner = new ClaudeBoardRunner({
    ...
    claimTask: ({ worker_id }) => {
      ...
      return {
        ...
        skill: 'saga-reviewer',
        repository: null,
        // execution_id omitted — see makeRunner note.
        execution_context: {
          authority: { allowed_saga_tools: ['task_get', 'worker_done'] },
        },
      };
    },
    getTaskState: id => states.get(id),
  ...
  try {
    runner.start({ projectId: 75, concurrency: 1 });
...
test('W5-A6 legacy preservation: resolveLaunchSpec returns null → legacy path', async () => {
  ...
  try {
    runner.start({ projectId: 76, concurrency: 1 });
...
test('W5-A6 robustness: resolveLaunchSpec that throws → legacy path (no crash)', async () => {
  ...
  try {
    runner.start({ projectId: 77, concurrency: 1 });
