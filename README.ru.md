...
- Все проекты как переключаемые доски (Backlog / In Progress / Review / Done / Blocked)
- По каждой карточке: исполнитель, возраст, эпик, тип задачи, live heartbeat
- Матрицу покрытия: какие AC реализованы и верифицированы
- Реестр приёмочных испытаний: статус верификации по эпизодам
- Кликабельные карточки → детали задачи
- Лента активности с пульсом heartbeat (зелёный/жёлтый/красный)
