...
function replaceOnce(file, needle, replacement) {
  ...
  if (found !== 1) throw new Error(`${file}: expected one anchor, found ${found}: ${needle.slice(0, 120)}`);
...
function replaceBetween(file, start, end, replacement) {
  ...
  if (starts !== 1) throw new Error(`${file}: start anchor count=${starts}: ${start.slice(0, 100)}`);
  const from = source.indexOf(start);
  const to = source.indexOf(end, from + start.length);
  if (to < 0) throw new Error(`${file}: end anchor missing: ${end.slice(0, 100)}`);
  write(file, source.slice(0, from) + replacement + source.slice(to));
}

const orchestrate = 'src/orchestrate.ts';

replaceOnce(orchestrate,
...
for (const [from, to] of [
  ['currentStage(epicId)', 'currentStage(epicId, opts)'],
  ['countActiveTasks(epicId)', 'countActiveTasks(epicId, opts)'],
  ['generateNextIfReady(epicId)', 'generateNextIfReady(epicId, opts)'],
  ['readLatestBriefDecision(epicId)', 'readLatestBriefDecision(epicId, opts)'],
  ['readEpisodeMeta(epicId)', 'readEpisodeMeta(epicId, opts)'],
  ['tryAdvanceStage(epicId)', 'tryAdvanceStage(epicId, opts)'],
  ['attemptHeal(epicId, stage, advance.error)', 'attemptHeal(epicId, stage, advance.error, opts)'],
  ['spawnGenericRecoveryTask(epicId, stage, advance.error)', 'spawnGenericRecoveryTask(epicId, stage, advance.error, opts)'],
  ['clearNeedsHuman(epicId)', 'clearNeedsHuman(epicId, opts)'],
  ['readTargetConcurrency(epicId, concurrency)', 'readTargetConcurrency(epicId, concurrency, opts)'],
]) {
  const source = read(orchestrate);
  if (!source.includes(from)) continue;
  write(orchestrate, source.split(from).join(to));
}
replaceOnce(orchestrate,
...
// Add transition sweep activity implementation to the SQLite task adapter.
replaceOnce(
...
// E2E uses the same concrete persistence adapters as the composition root.
replaceOnce(
...
// Characterization follows the moved persistence anchors.
replaceOnce(
