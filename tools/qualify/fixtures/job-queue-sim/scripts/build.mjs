/**
 * job-queue-sim/scripts/build.mjs - the deterministic build step: digests the
...
const manifest = { kind: 'job-queue-sim.build-manifest.v1', inputs: entries, buildDigest };

await mkdir(join(ROOT, 'dist'), { recursive: true });
await writeFile(join(ROOT, 'dist', 'build-manifest.json'), JSON.stringify(manifest, null, 2) + '\n');
process.stdout.write('job-queue-sim build: ' + buildDigest + '\n');
