/**
 * cli-stats/scripts/build.mjs - the deterministic build step: digests the
...
const manifest = { kind: 'cli-stats.build-manifest.v1', inputs: entries, buildDigest };

await mkdir(join(ROOT, 'dist'), { recursive: true });
await writeFile(join(ROOT, 'dist', 'build-manifest.json'), JSON.stringify(manifest, null, 2) + '\n');
process.stdout.write('cli-stats build: ' + buildDigest + '\n');
