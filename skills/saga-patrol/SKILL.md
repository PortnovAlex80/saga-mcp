...
#### 6e. Architecture express read

<!-- source: EXT-12 https://skillsdirectory.com/skills/rsmdt-codebase-exploration -->

Bounded (< 5 мин) проход по архитектуре — не глубокий реверс-инжиниринг, а снимок: где точки входа, какие top-level модули, куда направлены зависимости, есть ли очевидные запахи. Цель — дополнить §6a–6d (размер/типы/coverage) контуром «как устроено», чтобы в отчёте появилось одно-два предложения об архитектуре. Всё строго read-only.

Если кодовая фаза ещё не стартовала (см. оговорку в начале §6) — пропускаем 6e вместе с остальным шагом 6.

**1) Точки входа** — где система «входит в код»:

```bash
ROOT="<local_path>"
# CLI/бинарные точки входа: package.json bin/main fields
grep -E '"(bin|main|module|exports)"' "$ROOT/package.json" 2>/dev/null
# HTTP/routes точки входа (web-репо)
grep -rnE "(Route|path|endpoint|router\.|app\.(get|post|use))" "$ROOT/src" 2>/dev/null \
  --include='*.ts' --include='*.js' --include='*.tsx' | head -n 20
# Точки входа по конвенции имён
find "$ROOT/src" -maxdepth 3 \( -name 'index.*' -o -name 'main.*' -o -name 'server.*' \
                                -o -name 'app.*' -o -name 'cli.*' \) 2>/dev/null | head -n 20
```

**2) Top-level модули** — каркас, без погружения внутрь:

```bash
# Один уровень структуры src (или корня, если src нет)
SRC_DIR="$ROOT/src"; [ -d "$SRC_DIR" ] || SRC_DIR="$ROOT"
# Каталоги верхнего уровня + число файлов в каждом (где «тело» системы)
find "$SRC_DIR" -maxdepth 1 -type d | while read -r d; do
  n=$(find "$d" -type f \( -name '*.ts' -o -name '*.js' -o -name '*.tsx' \) | wc -l)
  printf '%4d  %s\n' "$n" "$(basename "$d")"
done | sort -rn | head -n 15
```

**3) Направление зависимостей** — кто от кого зависит (только направление, не полный граф):

```bash
# Какие локальные модули импортируются чаще всего (хабы)
grep -rhoE "from ['\"](\.\./|\./|@/|#/).*['\"]" "$SRC_DIR" \
  --include='*.ts' --include='*.tsx' 2>/dev/null \
  | sort | uniq -c | sort -rn | head -n 10
# Внешние зависимости, реально используемые в коде (не весь package.json)
grep -rhoE "from ['\"][^./][^'\"]*['\"]" "$SRC_DIR" \
  --include='*.ts' --include='*.tsx' 2>/dev/null \
  | sort -u | head -n 20
```

**4) Очевидные запахи** — то, что бросается в глаза за минуту:

```bash
# Бог-файлы (самые длинные исходники — кандидаты на разделение)
find "$SRC_DIR" -type f \( -name '*.ts' -o -name '*.tsx' -o -name '*.js' \) \
  -not -path '*/node_modules/*' -not -path '*/dist/*' \
  -exec wc -l {} + 2>/dev/null | sort -rn | head -n 10
# TODO/FIXME/HACK/XXX — незавершённое (плотность = «бой»)
grep -rnE "TODO|FIXME|HACK|XXX" "$SRC_DIR" --include='*.ts' --include='*.tsx' 2>/dev/null | wc -l
```

**Что считываем (одно-два предложения в отчёт):**
- **Точки входа:** сколько и где (CLI / HTTP / оба). Нет ни одного явного → каркас без «дверей», подозрительно.
- **Модули:** сколько top-level + самый «тяжёлый» по числу файлов. 1–2 монолитных каталога → плоская структура, нет декомпозиции.
- **Зависимости:** есть ли явный хаб-модуль (на него ссылается всё). Кольцевых импортов на этом уровне не видно — это только гипотеза для глубокого ревью.
- **Запахи:** бог-файл > 500 строк; высокая плотность TODO/FIXME → «бой» в коде, в ревью пойдёт с замечаниями.

**Read-only-сертификат (общий для §6e):** только `grep`/`find`/`wc`/`sort`/`uniq`/`head` над существующими файлами. Ничего не пишем, ничего не собираем, пакеты не качаем. Команды читают `src/` (или корень) как текст. Если `src/` нет — работаем по корню, но сообщаем об этом (нестандартная раскладка).

**Граница 6e (важно):** это **экспресс-контур**, не архитектурный аудит. Мы не строим полный dependency graph, не меряем coupling/cohesion, не оцениваем паттерны. Любой «запах» из 6e — повод для отдельного code review (`saga-code-reviewer`), а не финальный вердикт патруля. Глубокий реверс-инжиниринг (см. EXT-12) здесь не делаем — patrol читает архитектуру с высоты, как остальной срез.
...
- архитектура (6e, контур): точки входа=<N, CLI/HTTP>, top-level модулей=<N>, хаб=<имя/нет>, запахи=<бог-файл N строк / TODO=N / нет>
