...
function migrateArtifactTypes(db: Database.Database): void {
  ...
  // Detection predicate: the newest artifact type is 'SPEC'. If the live DDL
  // already includes it, the catalog is up-to-date and the migration is a no-op.
  // Older DBs (CHECK ends at 'OQ' or earlier) need the rebuild.
  if (!row?.sql || row.sql.includes("'SPEC'")) return;
