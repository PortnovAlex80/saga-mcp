/**
 * ProcessRun sole-writer repository (WP-06, plan phase EK-3).
 *
 * The ONLY module with direct SQL against the process_run table (the frozen
 * budget's lawfulRepositoryConvention: src/workflow-kernel/persistence/
 * <aggregate>-repository.ts). Owns exactly the seven ProcessRun commands of
...
import { ProcessRunReducer } from '../domain/reducers/process-run.js';
import {
...
const SELECT_HEADS = 'SELECT instance_id, revision, status, terminal FROM process_run';
const INSERT_HEAD =
  'INSERT INTO process_run (instance_id, aggregate, revision, status, terminal, last_sequence) VALUES (?, ?, ?, ?, ?, ?)';
const UPDATE_HEAD_CAS =
  'UPDATE process_run SET revision = ?, status = ?, terminal = ?, last_sequence = ? WHERE instance_id = ? AND revision = ?';
...
export class ProcessRunRepository {
  readonly aggregate = ProcessRunReducer.aggregate;
  ...
  /** All ProcessRun heads (sole lawful reader surface for other aggregates). */
  ...
  /** The exact ProcessRun head (ordinary commands read the exact owner). */
  ...
  applyCommand(input: CommandInput, options?: RepositoryApplyOptions): CommandOutcome {
    return applyCommandInOwnTransaction(
      this.db,
      ProcessRunReducer.ownedCommands,
      this.aggregate,
      input,
      {
