/**
 * LifecycleRun sole-writer repository (WP-06, plan phase EK-3).
 *
 * The ONLY module with direct SQL against the lifecycle_run table (the frozen
 * budget's lawfulRepositoryConvention: src/workflow-kernel/persistence/
 * <aggregate>-repository.ts). Owns exactly the seven LifecycleRun commands of
...
import { LifecycleRunReducer } from '../domain/reducers/lifecycle-run.js';
import {
...
const SELECT_HEADS = 'SELECT instance_id, revision, status, terminal FROM lifecycle_run';
const INSERT_HEAD =
  'INSERT INTO lifecycle_run (instance_id, aggregate, revision, status, terminal, last_sequence) VALUES (?, ?, ?, ?, ?, ?)';
const UPDATE_HEAD_CAS =
  'UPDATE lifecycle_run SET revision = ?, status = ?, terminal = ?, last_sequence = ? WHERE instance_id = ? AND revision = ?';
...
export class LifecycleRunRepository {
  readonly aggregate = LifecycleRunReducer.aggregate;
  ...
  /** All LifecycleRun heads (sole lawful reader surface for other aggregates). */
  ...
  /** The exact LifecycleRun head (ordinary commands read the exact owner). */
  ...
  applyCommand(input: CommandInput, options?: RepositoryApplyOptions): CommandOutcome {
    return applyCommandInOwnTransaction(
      this.db,
      LifecycleRunReducer.ownedCommands,
      this.aggregate,
      input,
      {
