/**
 * StageRun sole-writer repository (WP-06, plan phase EK-3).
 *
 * The ONLY module with direct SQL against the stage_run table (the frozen
 * budget's lawfulRepositoryConvention: src/workflow-kernel/persistence/
 * <aggregate>-repository.ts). Owns exactly the seven StageRun commands of
...
import { StageRunReducer } from '../domain/reducers/stage-run.js';
import {
...
const SELECT_HEADS = 'SELECT instance_id, revision, status, terminal FROM stage_run';
const INSERT_HEAD =
  'INSERT INTO stage_run (instance_id, aggregate, revision, status, terminal, last_sequence) VALUES (?, ?, ?, ?, ?, ?)';
const UPDATE_HEAD_CAS =
  'UPDATE stage_run SET revision = ?, status = ?, terminal = ?, last_sequence = ? WHERE instance_id = ? AND revision = ?';
...
export class StageRunRepository {
  readonly aggregate = StageRunReducer.aggregate;
  ...
  /** All StageRun heads (sole lawful reader surface for other aggregates). */
  ...
  /** The exact StageRun head (ordinary commands read the exact owner). */
