...
export const WORKSHOP_EXECUTABLE_CAPABILITIES: readonly ExecutableCapabilityManifestEntry[] = [
  ...
  // (parallelism anti-pattern + shared-surface extraction).
  checkProvider(DEVELOPMENT_REPLAN_GRAPH_CHECK_PROVIDER_ID, DEVELOPMENT_REPLAN_GRAPH_CHECK_PROVIDER_VERSION, DEVELOPMENT_REPLAN_GRAPH_CHECK_PROVIDER_DIGEST),
  ...
  // STAGE-18 R2 — the implementation claim-surface monotonicity ratchet
  // (live finding E-F5 / matrix E8): a card may not silently narrow its
  // own claimed file surface between submissions; the lawful exit is an
  // explicit snapshot.droppedFiles disposition.
  checkProvider(DEVELOPMENT_IMPLEMENTATION_CLAIM_MONOTONICITY_CHECK_PROVIDER_ID, DEVELOPMENT_IMPLEMENTATION_CLAIM_MONOTONICITY_CHECK_PROVIDER_VERSION, DEVELOPMENT_IMPLEMENTATION_CLAIM_MONOTONICITY_CHECK_PROVIDER_DIGEST),
