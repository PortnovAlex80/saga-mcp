 ...
 *
 * WAVE 6 STATUS (fourth audit 2026-08-02): the executor's `walk()` now builds
 * the legacy `ctx.frame` via `assembleFrameFromDurableNodeRuns` (a boundary
 ...
 * was FULLY REMOVED — `walk()` calls the adapter by name, and `restoreFrame`
 * is in the forbidden-fallback gate (no-execution-scoped-lookup.test.mjs).
 * This assembler remains the v2 no-fallback path (exact `ProductRef`
 * resolution, §9.11).
 *
