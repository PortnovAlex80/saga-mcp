...
`schema_version`, and `payload.target.*` pre-filled from the issued
certificate. Never reconstruct its path or copy a fresh template over it.
Only `Edit` that existing file.
...
2. `Read` the exact machine-provisioned diagnosis call file back.
...
### Step 6: Complete
After a durable diagnosis receipt:
```
worker_done({
  task_id: <integer>, worker_id: "<string>", execution_id: "<string>",
  result: "Diagnosis submitted (accepted|rejected). File: <exact machine-provisioned call path>."
})
```
