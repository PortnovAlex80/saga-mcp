 ...
 * - `provider` / `model` / `effort` are carried into spawn args only for
 *   model-backed executors; the simulator ignores them.
...
/**
 * Reduce a WorkerExecutionRoute to the legacy `ExecutionModelRoute` shape
 * ({provider, model, effort}) consumed by spawn-side code that has not yet been
 * migrated. Gateway provenance / spawn args read this until full cutover.
 */
export function routeToModelRoute(
  route: WorkerExecutionRoute,
): { provider: string; model: string | null; effort: string | null } {
  return {
    provider: route.provider?.id ?? 'zai',
